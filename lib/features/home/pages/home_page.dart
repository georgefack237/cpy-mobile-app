...
import 'package:cpy_app/features/hymns/providers/admin_hymn_book_provider.dart';
...
class _HomePageState extends State<HomePage>  with AutomaticKeepAliveClientMixin{
  ...
  void shouldUpdateDatabase() async {
   ...
   // if(adminProvider.dataResponse!.songCount != null && adminProvider.dataResponse!.songCount != provider.allSongs){

     // logger.i('Should run update now');
    //}else{
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Container(child: Icon(LucideIcons.messageCircle, color: Colors.white,size: 20,),
                      padding: EdgeInsets.all(10),
                      decoration: BoxDecoration(
                      ...
                      const Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text("Besoin d'aide ? ", style: TextStyle(color: Colors.black87, fontSize: 15)),

                          Text('Écrivez-nous', style: TextStyle(color: Colors.black45))
                        ],
                      ),
                  ...
                  IconButton(
                      onPressed: (){
                        ...
                        if (Platform.isAndroid) {
                          ...
                          launchUrl(
                              Uri.parse(
                                  'https://wa.me/$contact?text=Bonjour!'),
                              mode: LaunchMode.externalApplication);
                        } else {
                          launchUrl(
                              Uri.parse(
                                  'whatsapp://wa.me/$contact/?text=${Uri.encodeFull(message)}'),
                              mode: LaunchMode.externalApplication);

                        }

                      },
                      icon: Icon(LucideIcons.send, color: primaryDark,)
                  )
  ...
  Widget buildOfflineContent() {
    return Consumer<PictureVerseProvider>(
        builder: (context, provider, child) {
          ...
          return provider.verses != null ? BibleVerseCarousel(verses: provider.verses ?? []):
          provider.isLoading ?  Expanded(
              child: Container(
                height:  context.screenSize.height * .44,
                decoration: BoxDecoration(
                  borderRadius: BorderRadius.circular(16),
          ...
          provider.error != null ? Center(child: Text(provider.error!)): Shimmer.fromColors(
            baseColor: Colors.grey[300]!,
            highlightColor: Colors.grey[100]!,
            child: Container(
              height: context.screenSize.height * .44,
